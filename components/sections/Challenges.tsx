...
/**
 * Challenges — grid of pain-point cards. Server Component.
 */
export default function Challenges() {
  const isOddTotal = challenges.length % 2 !== 0

  return (
    <Section id="challenges" background="rgb(238,238,238)">
      {/* Background glow */}
      <div
        className="absolute pointer-events-none top-[-200px] left-1/2 -translate-x-1/2 w-full md:w-[800px] h-[800px]"
        style={{ background: 'radial-gradient(circle, rgba(0,0,0,.04) 0%, transparent 70%)' }}
      />
          ...
          return (
            <Card
              ...
              className={`challenge-card reveal reveal-d${(i % 5) + 1}
                ${isLast && isOddTotal ? 'md:col-span-2 md:max-w-md md:mx-auto' : ''}
              `}
            >
              {/* Corner glow */}
              <div
                className="absolute top-0 right-0 pointer-events-none w-[120px] h-[120px]"
                style={{ background: 'radial-gradient(circle, rgba(0,0,0,.03) 0%, transparent 70%)' }}
              />
              ...
              <h3 className="text-lg md:text-xl font-bold text-gray-800 mb-3">{challenge.title}</h3>
              <p className="text-sm md:text-base leading-relaxed text-gray-600">{challenge.text}</p>
            </Card>
          )
        })}
      </div>
