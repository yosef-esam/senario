...
export default function PlanCard({ plan }: PlanCardProps) {
  return (
    ...
    >
      {plan.featured && (
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 bg-gold text-teal-dark px-6 py-1.5 text-xs font-bold"
      ...
      <ul className="list-none text-right flex-1 mb-9 space-y-0">
        {plan.features.map((feature, i) => (
          ...
          >
            <span className="font-bold text-xs flex-shrink-0 mt-0.5 text-teal-500">✓</span>
            {feature}
          </li>
        ))}
      </ul>
